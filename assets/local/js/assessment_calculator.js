...
$('input[name="total_units"]').on('keyup', function() {
...
$('input[name="other_fees"]').on('keyup', function() { calculateMiscellaneousFee(); calculateBalance(); });
$('input[name="payment_stated"]').on('keyup', function() { calculateBalance(); });
...
var getHiddenAmount = function(name) {
	var amount = Number($('input[name="'+name+'"]').val());
